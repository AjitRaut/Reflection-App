...
const createFeedback = async (req, res) => {
  try {
    ...
    const feedbackLink = `http://localhost:5173/feedback/${feedbackName.replace(
      /\s+/g,
      "-"
    )}`;
    ...
    res
      .status(201)
      .json({ message: "Feedback link created successfully!", feedbackLink });
  } catch (error) {
    if (error.code === 11000) {
...
const submitFeedback = async (req, res) => {
  try {
    ...
    if (!positiveEmotions.length && !improvementAreas.length) {
      return res
        .status(400)
        .json({ error: "At least one option must be selected." });
    }
